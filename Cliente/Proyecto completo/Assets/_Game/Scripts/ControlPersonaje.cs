...
public class ControlPersonaje : MonoBehaviour {
	...
	public void Morir ()
	{
		...
		if(esLocalPlayer) 
		{
			NManayer.instance.ReactivarCanvasPorMuerte();
		}
	...
	public void Rotar(float i)
	{
		if(i!=0) sRot.value = 0;
	}

	public void BTNDisparar()
	{
		NManayer.instance.CommandShoot();
		Disparar();
	}

	public void BTNBailar()
	{
...
//		animPersonaje.SetFloat ("velocidad", Input.GetAxis ("Vertical"));
		...
		transform.Translate (Vector3.forward * velocidad * (Input.GetAxis ("Vertical") + sMov.value) *Time.deltaTime);
		transform.Rotate (Vector3.up * velRotacion * (Input.GetAxis ("Horizontal") + sRot.value)*Time.deltaTime);
